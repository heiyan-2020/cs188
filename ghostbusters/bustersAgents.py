...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        min_action = (9999, Directions.STOP)
        for distribution in livingGhostPositionDistributions:
            believeGhostPos = distribution.argMax()
            believeMinDist = min([(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), believeGhostPos), action) for action in legal])
            if believeMinDist[0] < min_action[0]:
                min_action = believeMinDist
