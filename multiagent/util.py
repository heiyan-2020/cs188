...
def mazeDistance(xy1, xy2, walls):
    fringe = Queue()
    visited = []
    fringe.push(xy1)
    layers = 0
    ...
    while not fringe.isEmpty():
        layerNodes = []
        while not fringe.isEmpty():
            layerNodes.append(fringe.pop())
        for node in layerNodes:
            if node == xy2:
                return layers
            for delta in deltas:
                newPos = (node[0] + delta[0], node[1] + delta[1])
                if not walls[newPos[0]][newPos[1]] and not newPos in visited:
